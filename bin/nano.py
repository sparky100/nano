...
def write_config_file_cb(b):
    path_to_share = os.path.join('~', '.local','share','nano')
...
def get_config_files():
    ...
    if nanoHUB_flag:
        full_path = os.path.expanduser("~/data/results/.submit_cache/nano")  # does Windows like this?
    else:
        # local cache
        try:
            cachedir = os.environ['CACHEDIR']
            full_path = os.path.join(cachedir, "nano")
        except:
...
def run_done_func(s, rdir):
    ...
    if nanoHUB_flag:
        ...
        os.system("submit  mail2self -s 'nanoHUB nano' -t 'Your Run completed.'&")
...
tool_title = widgets.Label(r'\(\textbf{nano}\)')
